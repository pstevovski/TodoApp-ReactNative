...
const TodoItemsList = (props: TodoItemsListProps) => {
  ...
  const getListData = async () => {
    ...
    if (list !== null) {
      setCurrentList(
        JSON.parse(list).find((list: any) => list.id === props.id),
      );
  ...
  const getDataFromList = async () => {
    const todoItemIndex = currentList.children.findIndex(
      (todo: any) => todo.todoID === itemID,
    );
    const bookmarkedItemIndex = currentList.bookmarked.findIndex(
      (todo: any) => todo.todoID === itemID,
    );
    const selectedItem = currentList.children.find(
      (item: any) => item.todoID === itemID,
    );
    const currentListIndex = currentList.findIndex(
      (list: any) => list.id === props.id,
    );
  ...
  const updateCurrentList = async (action: string) => {
    ...
    switch (true) {
      case action === 'MARK_COMPLETE':
        const checkIfEveryTodoIsCompleted = currentList.children.every(
  ...
  const bookmarkTodo = async () => {
    ...
    if (
      !currentList[0].bookmarked.find(
        (bookmark: any) => bookmark.todoID === itemID,
      )
    ) {
  ...
  const openMenu = async (id: string) => {
    ...
    if (bookmarks) {
      const bookmarksList = JSON.parse(bookmarks).find(
        (list: any) => list.id === props.id,
      ).bookmarked;
      const checkIfBookmarkExists = bookmarksList.find(
        (bookmark: any) => bookmark.todoID === id,
      );
  ...
  const onRefresh = async () => {
    ...
    console.log(
      ...
      currentList.children.some((item: any) => !item.completed),
    );
  };
